	...
	Если ИменаПредметовДляФункций <> Неопределено Тогда
		Если ИменаПредметовДляФункций.Количество() > 0 Тогда
			Для Каждого ИмяПредмета Из ИменаПредметовДляФункций Цикл
				...
				//ЕАЕ Добавляем новую автоподстановку "Подписант документа" для использования при запуске бизнес-процессов ++ 
				ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.ЕАЕ_ПодписантДокумента(Объект, ИмяПредмета)",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.Подписант документа'"),Строка(ИмяПредмета)));
				//--
...
// ЕАЕ Возвращает подписанта документа
//
// Параметры:
//   БизнесПроцессОбъект - БизнесПроцессОбъект.<Тип бизнес процесса> - процесс, для которого
//                         вычисляется автоподстановка
...
//   СправочникСсылка.Пользователи
//
Функция ЕАЕ_ПодписантДокумента(БизнесПроцессОбъект, ИмяПредмета) Экспорт
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		ЕстьПодписант = Ложь;
		Если ТипЗнч(СтрокаПредмета.Предмет) = Тип("СправочникСсылка.ИсходящиеДокументы") Тогда 
			ЕстьПодписант = Истина;
		ИначеЕсли ТипЗнч(СтрокаПредмета.Предмет) = Тип("СправочникСсылка.ВнутренниеДокументы") Тогда 
			ЕстьПодписант = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПредмета.Предмет, "ВидДокумента.ИспользоватьПодписание");
		КонецЕсли;	
		
		Если Не ЕстьПодписант Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Автоподстановка не может быть выполнена для предмета ""%1"" процесса.'"),
				Строка(ИмяПредмета));
		КонецЕсли;
	
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПредмета.Предмет, "Подписал");
